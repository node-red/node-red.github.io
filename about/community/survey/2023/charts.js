...
RED.chart.histogram = function(opts) {
    ...
    if (opts.label || opts.previousLabel) {
        const keyLabel = $('<div class="chart-label">').appendTo(target);
        function buildKey(value, swatchClass) {
            const keySpan = $('<span>').addClass('chart-key')
            $('<span>').addClass("chart-key-swatch").addClass(swatchClass).appendTo(keySpan)
            $('<span>').text(value).appendTo(keySpan)
            return keySpan
        }
        if (opts.previousLabel) {
            buildKey(opts.previousLabel, 'chart-bar-fill-series-previous').appendTo(keyLabel)
        }
        if (opts.label) {
            buildKey(opts.label, 'chart-bar-fill-series-0').appendTo(keyLabel)
        }
    }
    ...
    data.forEach(function(d) {
        ...
        if (d.value !== undefined) {
            ...
            $('<div class="popover">').html((opts.label?`${opts.label}: `:'') + formatValue(d.value)).appendTo(c);
            if (d.previous !== undefined) {
                bar.addClass("chart-bar-previous")
                var c = $("<span>").addClass("chart-bar-fill chart-bar-fill-series-previous").width(d.previous+"%").prependTo(bar);
                $('<div class="popover">').html(opts.previousLabel + ': ' + formatValue(d.previous)).appendTo(c);
            }
